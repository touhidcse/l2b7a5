...
import { BookingItemProps, IBooking, PaymentItemProps, ReviewItemProps, SectionProps } from '@/lib/types'
...
export function BookingItemFull({ booking }: { booking: IBooking }) {
    ...
    const getDisplayStatus = (status: IBooking['status']): string => {
        ...
        <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800 rounded-lg mb-2 last:mb-0">
            <div className="flex-1">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                    {booking.service?.category?.type || 'Service'} - {new Date(booking.bookingDate).toLocaleDateString()}
                </p>
                <p className="text-xs text-slate-500 dark:text-slate-400">
